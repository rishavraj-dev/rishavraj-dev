# Hi there, I'm Rishav Raj! 👋

### 🎓 B.Tech CSE (Core) Student @ VIT Bhopal University

I am a passionate first-year Computer Science student exploring the world of **Web Development**, **Python**, and **Software Engineering**. I love building tools that solve real-world problems, from educational platforms to healthcare solutions.

---

## 🚀 Featured Projects

Here are some of the key projects I've been working on, along with their live deployments:

### 📚 **[Studycore](https://github.com/rishavraj-dev/Studycore)**
> **[🌐 Launch Live Demo](https://studycore.kesug.com)**
> *The main iteration of my educational platform.*

A comprehensive web-based learning platform designed to streamline study materials and content delivery.
* **Tech Stack:** PHP, HTML, CSS
* **Focus:** Content management, user-friendly interface, and educational resource distribution.

### 💬 **[Webchat](https://github.com/rishavraj-dev/Webchat)**
> **[🌐 Launch Live Demo](https://webchat.kesug.com)**
> *Real-time communication made simple.*

A lightweight online chatting application that uses web polling to facilitate messaging.
* **Tech Stack:** PHP, Web Polling
* **Key Features:** Simple message pulling, real-time interface updates.

### 🏥 **[MediConnect](https://github.com/rishavraj-dev/mediconnect)**
> **[🌐 Launch Live Demo](https://mediconnect-rishav.onrender.com)**
> *Bridging the gap in rural healthcare.*

A healthcare access platform aimed at connecting rural patients with medical facilities. Developed during the **VITB-JHU Health Hackathon**.
* **Tech Stack:** HTML, CSS, JavaScript,Python
* **Mission:** Improving healthcare accessibility through technology.

---

## 🎓 University & Academic Projects

### 🏥 **[Hospital Management System](https://github.com/rishavraj-dev/HospitalManagementSystem)**
> *VIT Bhopal Course Project*

A robust Python-based application for managing hospital records.
* **Context:** Developed as a "Vidyarthi" / College Project for my 1st semester CSE project.
* **Features:** Patient record management, doctor scheduling, and administrative controls.
* **Tech Stack:** Python

---

## 📂 Archives & Legacy

* **[Studycore-legacy](https://github.com/rishavraj-dev/Studycore-leagacy)**: The foundational version of the Studycore project.
    * **[🌐 View Version 2 Live](https://studycore.infinityfreeapp.com)**: A hosted archive of the second iteration, featuring the original UI and lecture players.

---

## 🛠 Languages & Tools

<p align="left">
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white" />
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white" />
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white" />
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white" />
</p>

---

### 📫 Connect with me
* **GitHub:** [@rishavraj-dev](https://github.com/rishavraj-dev)
* **Email:** [raj.rishav@outlook.in](mailto:raj.rishav@outlook.in)
